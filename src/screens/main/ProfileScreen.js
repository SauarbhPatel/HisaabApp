  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={COLORS.gradientGreen} start={{ x: 0.13, y: 0 }} end={{ x: 1, y: 1 }}
        ...
        <Text style={styles.profEmail}>{user.email} · {user.role}</Text>
        <View style={styles.statPills}>
          {[['7 Projects', null], ['4 Devs', null], ['3 Clients', null]].map(([l]) => (
            <View key={l} style={styles.statPill}><Text style={styles.statPillText}>{l}</Text></View>
          ))}
        </View>
      </LinearGradient>
        ...
        <View style={styles.body}>
          <View style={styles.settingsGroup}>
            ...
            <SettingRow icon="👨‍💻" title="My Developers" sub="4 developers · Tap to manage"
              ...
              right={<Text style={[styles.settingArrow, { color: COLORS.primary }]}>›</Text>} />
          ...
          <View style={styles.settingsGroup}>
            <SettingRow icon="🔔" title="Notifications" sub="Reminders & alerts" right={<Toggle />} />
            <SettingRow icon="💰" title="Budget Alerts" sub="Alert at 80% of budget" right={<Toggle />} />
          </View>
...
const styles = StyleSheet.create({
  ...
  profAvatarText: { fontFamily: FONTS.nunito.black, fontSize: 26, color: '#fff' },
  profName: { fontFamily: FONTS.nunito.black, fontSize: SIZES.xl3, color: '#fff' },
  profEmail: { fontFamily: FONTS.dmSans.regular, fontSize: SIZES.base, color: 'rgba(255,255,255,0.8)', marginTop: 3 },
  statPills: { flexDirection: 'row', gap: 10, marginTop: 14 },
  statPill: { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 6 },
  statPillText: { fontFamily: FONTS.nunito.bold, fontSize: SIZES.base, color: '#fff' },
  scroll: { flex: 1 },
  body: { padding: 14 },
  settingsGroup: {
    backgroundColor: COLORS.card, borderRadius: 16,
    overflow: 'hidden', marginBottom: 14, ...SHADOWS.card,
  },
  settingRow: {
  ...
  settingTitle: { fontFamily: FONTS.nunito.bold, fontSize: SIZES.md, color: COLORS.text },
  settingSub: { fontFamily: FONTS.dmSans.regular, fontSize: SIZES.sm2, color: COLORS.text2, marginTop: 1 },
  settingArrow: { fontSize: SIZES.md2, color: COLORS.text3 },
  toggle: {
    width: 38, height: 22, borderRadius: 11,
    backgroundColor: COLORS.primary,
    alignItems: 'flex-end', justifyContent: 'center', paddingRight: 3,
  },
  logoutBtn: { alignItems: 'center', padding: 10 },
  logoutText: { fontFamily: FONTS.nunito.bold, fontSize: SIZES.base, color: COLORS.danger },
});
